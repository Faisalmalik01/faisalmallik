...
export default function Navbar({ view, setView, activeNav, setActiveNav }) {
  const goToWork = () => {
    ...
    setActiveNav("work");
    setTimeout(() => scrollToSection("work"), 20);
  };
  ...
  const goToLogo = () => {
    setView("work");
    setActiveNav(null);
    setTimeout(() => scrollToSection("hero"), 20);
  };
  ...
  return (
    ...
    >
      <div className="w-full px-5 sm:px-8 h-12 sm:h-14 flex items-center justify-between">
        {/* Signature logo */}
        <button onClick={goToLogo} className="flex items-center shrink-0">
          <img
        ...
        <nav className="flex items-center gap-0.5 sm:gap-1">
          {[
            { key: "work", label: "WORK", action: goToWork },
            { key: "skills", label: "SKILLS", action: goToSkills },
            { key: "about", label: "ABOUT", action: goToAbout },
          ].map((link) => (
            <button
              key={link.key}
              onClick={link.action}
              ...
              style={{
                color: activeNav === link.key ? "var(--ink)" : "var(--muted)",
                background: activeNav === link.key ? "var(--panel)" : "transparent",
              }}
            >
              {link.label}
            </button>
